...
write.csv(data.frame(retina_zinbwave@reducedDims$zinbwave), file = 'dev/Rmethods_out/retina_ZINB-WAVE.csv', row.names = F) # retina data
```
...
SIMLR requires an expected number of clusters in the dataset.  We can use SIMLR to estimate this.
```{r}
...
print(which.min(colon_clu_est$K1))
```

Now we can normalize our counts data and feed it into the SIMLR algorithm with the expected number of clusters from above.
```{r}
...
print(which.min(retina_clu_est$K1))
