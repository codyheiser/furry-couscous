...
def NMFreg(
    ...
    puckcounts_scaled = StandardScaler(with_mean=False).fit_transform(
        puckcounts_cellnorm
    )
    ...
    Hs_norm = StandardScaler(with_mean=False).fit_transform(Hs.drop("barcode", axis=1))
...
def deconv_factor_to_celltype_mean(row, adict, K, num_atlas_clusters):
    ...
    num_fact = list(
        collections.OrderedDict(
            sorted(collections.Counter(adict.values()).items())
        ).values()
    )
...
def cell_deconv(collapse):
    if collapse == "l2":
        tmp_df = Ha_norm.drop("cellname", axis=1).apply(
            lambda x: deconv_factor_to_celltype_l2(
                row=x,
                adict=factor_to_celltype_dict,
                K=K,
                num_atlas_clusters=num_atlas_clusters,
    ...
    if collapse == "sum":
        tmp_df = Ha_norm.drop("cellname", axis=1).apply(
            lambda x: deconv_factor_to_celltype_sum(
                row=x,
                adict=factor_to_celltype_dict,
                K=K,
                num_atlas_clusters=num_atlas_clusters,
    ...
    if collapse == "mean":
        tmp_df = Ha_norm.drop("cellname", axis=1).apply(
            lambda x: deconv_factor_to_celltype_mean(
                row=x,
                adict=factor_to_celltype_dict,
                K=K,
                num_atlas_clusters=num_atlas_clusters,
    ...
    mismatch_df = cell_clusters[
        cell_maxct_df["max_cell_type"] != cell_clusters["cluster"]
    ]
    print(
        "num mismatched: {}".format(
            cell_clusters[
                cell_maxct_df["max_cell_type"] != cell_clusters["cluster"]
            ].shape[0]
        )
...
def plot_bar_cellt(cell_deconv_df_norm, cell_maxct_df, metacell_dict):
    for key, value in metacell_dict.items():
        ...
        plt.figure(figsize(4, 4))
        plt.bar(
            x=range(int(num_atlas_clusters)),
...
def plot_boolean(size, coords, bead_maxct_df, plot_size_dict, metacell_dict):
    for key, value in metacell_dict.items():
...
def plot_boolean_thresh(
    size, coords, bead_maxct_df, bead_deconv_df_norm, plot_size_dict, metacell_dict
):
...
def plot_ct_loadings(coords, size, puckcounts, bead_deconv_df, plot_size_dict):
    ...
    bead_deconv_df_norm = np.true_divide(
        bead_deconv_df.drop("barcode", axis=1), barcode_totalloading[:, None]
    )
...
def plot_allct(
    coords, size, bead_deconv_df_norm, bead_maxct_df, plot_size_dict, num_atlas_clusters
):
    ...
    facet = sns.lmplot(
        ...
        size=10,
        scatter_kws={"s": 2 * plot_size_dict[size]},
    )
    # add a legend
    leg = facet.ax.legend(bbox_to_anchor=[1, 0.75], title="label", fancybox=True)
...
def plot_certainty_thresh(coords, size, bead_deconv_df_norm, plot_size_dict):
    ...
    expr = round(
        100
        * np.divide(
            coords[bead_deconv_df_norm["thresh_ct"] != 0].shape[0], coords.shape[0]
        ),
        2,
    )
...
def plot_certainty_perct(
    coords, size, bead_deconv_df_norm, df_clust, plot_size_dict, metacell_dict
):
    for key, value in metacell_dict.items():
...
def plot_certainty_perct_thresh(
    coords,
    size,
    bead_deconv_df_norm,
    df_clust,
    ...
    for key, value in metacell_dict.items():
        ...
        ct_df = bead_deconv_df_norm[bool_col]
        ct_df["col"] = ct_df["maxval"].apply(
            lambda x: 0 if x <= thresh_certainty[int(key) - 1] else x
        )
        keep_thresh_df[int(key) - 1] = ct_df["maxval"] > thresh_certainty[int(key) - 1]
        remove_thresh_df[int(key) - 1] = (
            ct_df["maxval"] <= thresh_certainty[int(key) - 1]
        )
...
def plot_pie(
    ...
    for key, value in metacell_dict.items():
        ...
        title_str = "{}um, {} purity threshold {}".format(
            size, value, round(thresh_cert, 4)
        )
        plt.title(title_str)
        plt.show()
...
def plot_bar_ct(bead_deconv_df_norm, df_clust, size, metacell_dict):
    for key, value in metacell_dict.items():
        ...
        plt.bar(
            x=range(int(num_atlas_clusters)),
            height=np.sum(
                ct_df.drop(["barcode", "max_cell_type", "maxval", "thresh_ct"], axis=1),
                axis=0,
            ),
            tick_label=list(metacell_dict.values()),
        )
...
def plot_allct_thresh(
    coords, size, bead_deconv_df_norm, bead_maxct_df, plot_size_dict, metacell_dict
):
    bool_col = bead_deconv_df_norm["thresh_ct"] == 0
    plt.figure(figsize(12, 12))
    for key, value in metacell_dict.items():
