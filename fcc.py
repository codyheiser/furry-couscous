...
from scipy.spatial.distance import (
    pdist,
    cdist,
)  # unique pairwise and crosswise distances
from scipy.interpolate import interpnd, griddata
from ot import (
    wasserstein_1d,
)  # POT implementation of Wasserstein distance between 1D arrays
...
class couscous:
    ...
    def __init__(
        self, data, data_type="counts", labels=[0, 0], cells_axis=0, barcodes=None
    ):
        """
        initialize object
        ...
        if data_type is not "counts":
            self.clu[data_type] = Cluster(
                np.ascontiguousarray(self.data[data_type]), autoplot=False
    ...
    def arcsinh_norm(self, data_type="counts", norm="l1", scale=1000, ranks="all"):
        ...
        if not norm:
            ...
            out = np.arcsinh(
                normalize(np.ascontiguousarray(self.data[data_type]), axis=1, norm=norm)
                * scale
            )
    ...
    def log_norm(self, data_type="counts", norm="l1", ranks="all"):
        ...
        if not norm:
            out = np.log1p(
                np.ascontiguousarray(self.data[data_type])
    ...
    def knn_graph(self, k, data_type="counts", **kwargs):
        ...
        return kneighbors_graph(
            self.dist_matrix(data_type=data_type, **kwargs),
            k,
            mode="connectivity",
            include_self=False,
        ).toarray()
    ...
    def fcc_PCA(self, n_components, data_type="counts", transform=None, **kwargs):
        ...
        self.PCA_fit = PCA(n_components=n_components).fit(
            transformed
        )  # fit PCA to data
        self.data["PCA"] = self.PCA_fit.transform(
            transformed
        )  # transform data to PCA space and save in data attribute
        self.clu["PCA"] = Cluster(
            self.data["PCA"].astype("double"), autoplot=False
        )  # perform DPC on PCA results

    def fcc_tSNE(
        self, perplexity, data_type="counts", transform=None, seed=None, **kwargs
    ):
        """
        ...
        self.clu["t-SNE"] = Cluster(
            self.data["t-SNE"].astype("double"), autoplot=False
        )  # perform DPC on t-SNE results
    ...
    def fcc_UMAP(
        ...
        self.clu["UMAP"] = Cluster(
            self.data["UMAP"].astype("double"), autoplot=False
        )  # perform DPC on UMAP results
    ...
    def silhouette_score(self, data_type):
        ...
        """
        assert hasattr(
            self.clu[data_type], "membership"
        ), "Clustering not yet determined. Assign clusters with self.clu.assign().\n"
        return silhouette_score(
            self.data[data_type], self.clu[data_type].membership
    ...
    def plot_clusters(self, data_type):
        ...
        """
        assert hasattr(
            self.clu[data_type], "clusters"
        ), "Clustering not yet determined. Assign clusters with self.clu.assign().\n"
        fig, ax = plt.subplots(1, 3, figsize=(15, 5))
        ax[0].scatter(
            self.data[data_type][:, 0], self.data[data_type][:, 1], s=75, alpha=0.7
        )
        ax[0].scatter(
            self.data[data_type][self.clu[data_type].clusters, 0],
            self.data[data_type][self.clu[data_type].clusters, 1],
            s=90,
            c="red",
        )
        ax[1].scatter(
            self.data[data_type][:, 0],
            self.data[data_type][:, 1],
            s=75,
            alpha=0.7,
            c=self.clu[data_type].density,
        )
        ax[2].scatter(
            self.data[data_type][:, 0],
            self.data[data_type][:, 1],
            s=75,
            alpha=0.7,
            c=self.clu[data_type].membership,
            cmap=plt.cm.plasma,
        )
        IDs, counts = np.unique(
            self.clu[data_type].membership, return_counts=True
    ...
    def plot(
        ...
        **kwargs
        ...
        if color is None and features is None:
            ...
            if transform is None:
                mapper = pd.DataFrame(
                    self.data[feature_type], index=self.data["counts"].index
                )  # coerce to pd.DF with bead ID index

            elif transform == "arcsinh":
                mapper = pd.DataFrame(
                    self.arcsinh_norm(data_type=feature_type, **kwargs),
                    index=self.data["counts"].index,
                )  # coerce to pd.DF with bead ID index

            elif transform == "log1p":
                mapper = pd.DataFrame(
                    self.log_norm(data_type=feature_type, **kwargs),
                    index=self.data["counts"].index,
                )  # coerce to pd.DF with bead ID index

            elif transform == "gficf":
                mapper = pd.DataFrame(
                    self.gficf_norm(data_type=feature_type, **kwargs),
                    index=self.data["counts"].index,
                )  # coerce to pd.DF with bead ID index
            ...
            # if 'features' is a string, treat it as regex value
                ...
                color = mapper.loc[:, self.feature_IDs.str.contains(features)].sum(
                    axis=1
                )
            ...
            # if 'features' is a list of strings, treat them as regex values and return sum of (normalized) expression
                ...
                color = mapper.loc[
                    :, self.feature_IDs.str.contains("|".join(features))
                ].sum(axis=1)
        ...
        plotter = np.ascontiguousarray(
            self.data[data_type]
        )  # coerce data to np array for plotting
        ...
        DR_plot(dim_name=dim_name, figsize=figsize).plot(
            data=plotter, color=color, pt_size=pt_size, legend=legend, save_to=save_to
        )
    ...
    def plot_PCA(self, color=None, pt_size=75, save_to=None, figsize=(10, 5)):
        ...
        plt.plot(
            np.cumsum(
                np.round(self.PCA_fit.explained_variance_ratio_, decimals=3) * 100
            )
        )
    ...
    @classmethod
    def from_file(
        cls, datafile, data_type="counts", labels=[0, 0], cells_axis=0, barcodefile=None
    ):
        """
        ...
        if cells_axis == 1:
            ...
            labels = labels[
                ::-1
            ]  # return labels in reverse order to capture cell and feature IDs properly
    ...
    def drop_set(cls, counts_obj, drop_index, axis, data_type="counts", num=False):
        ...
        if not num:
            return cls(
                counts_obj.data[data_type].drop(drop_index, axis=axis),
                data_type=data_type,
                labels=[counts_obj.cell_labels, counts_obj.feature_labels],
                barcodes=codes,
            )

        elif axis == 1:
            return cls(
                counts_obj.data[data_type].drop(
                    counts_obj.data.columns[drop_index], axis=axis
                ),
                data_type=data_type,
                labels=[counts_obj.cell_labels, counts_obj.feature_labels],
                barcodes=codes,
            ...
            return cls(
                counts_obj.data[data_type].drop(
                    counts_obj.data.index[drop_index], axis=axis
                ),
                data_type=data_type,
                labels=[counts_obj.cell_labels, counts_obj.feature_labels],
                barcodes=codes,
    ...
    def keep_set(cls, counts_obj, keep_index, axis, data_type="counts", num=False):
        ...
        if not num:
            if axis == 0:
                return cls(
                    counts_obj.data[data_type].loc[keep_index, :],
                    data_type=data_type,
                    labels=[counts_obj.cell_labels, counts_obj.feature_labels],
                    barcodes=codes,
                )
            ...
            if axis == 0:
                return cls(
                    counts_obj.data[data_type].iloc[keep_index, :],
                    data_type=data_type,
                    labels=[counts_obj.cell_labels, counts_obj.feature_labels],
                    barcodes=codes,
                )
    ...
    def nvr_select(cls, counts_obj, parse_noise=True, data_type="counts", **kwargs):
        ...
        """
        if parse_noise:
            hqGenes = nvr.parseNoise(
                np.ascontiguousarray(counts_obj.data[data_type])
            )  # identify non-noisy genes
            selected_genes = nvr.select_genes(
                counts_obj.arcsinh_norm(data_type=data_type, **kwargs)[:, hqGenes]
            )  # select features from arsinh-transformed, non-noisy data

        else:
            selected_genes = nvr.select_genes(
                counts_obj.arcsinh_norm(data_type=data_type, **kwargs)
            )  # select features from arsinh-transformed, non-noisy data
        ...
        return cls(
            counts_obj.data[data_type].iloc[:, selected_genes],
            data_type=data_type,
            labels=[counts_obj.cell_labels, counts_obj.feature_labels],
            barcodes=codes,
        )
    ...
    def var_select(cls, counts_obj, n_features, data_type="counts"):
        ...
        """
        v = (
            counts_obj.data[data_type].var(axis=0).nlargest(n_features).index
    ...
    def downsample_rand(cls, counts_obj, n_cells, data_type="counts", seed=None):
        ...
        cells_out = np.random.choice(
            counts_obj.data[data_type].shape[0], n_cells, replace=False
        )
        ...
        return cls(
            counts_obj.data[data_type].iloc[cells_out],
            labels=[counts_obj.cell_labels, counts_obj.feature_labels],
            barcodes=codes,
        )

    @classmethod
    def downsample_proportional(
        cls, counts_obj, n_cells, data_type="counts", seed=None
    ):
        """
        ...
        IDs, clu_counts = np.unique(
            counts_obj.clu[data_type].membership, return_counts=True
        ...
        for ID, count in zip(IDs, clu_counts):
            ...
            cells_out = np.append(
                cells_out,
                np.random.choice(
                    np.where(counts_obj.clu[data_type].membership == ID)[0],
                    clu_num,
                    replace=False,
                ),
            )
...
class pita(couscous):
    ...
    def __init__(
        ...
        couscous.__init__(
            ...
            barcodes=barcodes,
        )  # inherits from DR object

        self.data["slide-seq"] = bead_locs  # store bead locations as attribute
        self.data["slide-seq"].sort_values(
            axis=0, by=["xcoord", "ycoord"], inplace=True
        )  # sort beads by image coordinates
        self.clu["slide-seq"] = Cluster(
            np.ascontiguousarray(self.data["slide-seq"].astype("double")),
            autoplot=False,
        )  # perform DPC on slide-seq positions for visualization by plot()
    ...
    def filter_beads(self, data_type="counts"):
        ...
        """
        self.data["slide-seq"] = self.data["slide-seq"].drop(
            self.data["slide-seq"].index.difference(self.data[data_type].index)
        )

    def snippet(
        self, xmin, xmax, ymin, ymax
    ):  # TODO: make this a classmethod or extensible to all DRs of pita
        """
        take snippet of image and make new pita object
        """
        self.data["slide-seq"] = self.data["slide-seq"].loc[
            (self.data["slide-seq"].xcoord > xmin)
            & (self.data["slide-seq"].xcoord < xmax)
            & (self.data["slide-seq"].ycoord > ymin)
            & (self.data["slide-seq"].ycoord < ymax)
        ]
    ...
    def map_pixels(self):
        ...
        self.pixel_map = griddata(
            self.data["slide-seq"].values,
            self.data["slide-seq"].index,
    ...
    def trim_pixels(self, threshold=100):
        ...
        xi = interpnd._ndim_coords_from_arrays(
            (self.grid_x, self.grid_y), ndim=self.data["slide-seq"].shape[1]
        )
        ...
        self.show_pita(
            assembled=bin_threshold(dists, threshmax=threshold), figsize=(4, 4)
        )
    ...
    def assemble_pita(
        ...
        **kwargs
        ...
        if transform == "arcsinh":
            mapper = pd.DataFrame(
                self.arcsinh_norm(data_type=feature_type),
                index=self.data["counts"].index,
            )  # coerce to pd.DF with bead ID index

        elif transform == "log1p":
            mapper = pd.DataFrame(
                self.log_norm(data_type=feature_type), index=self.data["counts"].index
            )  # coerce to pd.DF with bead ID index

        elif transform == "gficf":
            mapper = pd.DataFrame(
                self.gficf_norm(data_type=feature_type), index=self.data["counts"].index
            )  # coerce to pd.DF with bead ID index

        # determine which reference pixel map to use
        if trimmed:
            assert (
                self.pixel_map_trim is not None
            ), "Pixel map not trimmed. Run self.trim_pixels(), or set trimmed=False to generate image.\n"
        ...
        # if 'features' is a string, treat it as regex value
            ...
            map_features = mapper.loc[:, self.feature_IDs.str.contains(features)].sum(
                axis=1
            )
        ...
        # if 'features' is a list of integers, treat them as column indices and return sum of (normalized) expression
            ...
            raise ValueError(
                'Please provide features as a list of strings or integers. e.g. [0,3,4], ["Vim","Aldoc","Ttr"].'
            )

        assembled = np.array(
            [map_features.reindex(index=ref[x], copy=True) for x in range(len(ref))]
        )
    ...
    def show_pita(self, assembled, figsize=(7, 7), save_to=None, **kwargs):
        ...
        plt.figure(figsize=figsize)
        plt.imshow(
            assembled.T, origin="lower", **kwargs
        )  # transpose matrix and origin=='lower' to show image with positive axes starting from lower left
        plt.tick_params(labelbottom=False, labelleft=False)
    ...
    def from_file(
        ...
        if ".csv" in beadfile:
            ...
            beads = pd.read_csv(
                beadfile, header=labels[1], index_col=labels[0], sep="\t"
            )
...
def cluster_arrangement(
    ...
    dist_0_1 = pre_obj.barcode_distance_matrix(
        data_type=pre_type, ranks=[clusters[0], clusters[1]], transform=pre_transform
    ).flatten()
    dist_0_2 = pre_obj.barcode_distance_matrix(
        data_type=pre_type, ranks=[clusters[0], clusters[2]], transform=pre_transform
    ).flatten()
    dist_1_2 = pre_obj.barcode_distance_matrix(
        data_type=pre_type, ranks=[clusters[1], clusters[2]], transform=pre_transform
    ).flatten()
    ...
    post_0_1 = post_obj.barcode_distance_matrix(
        data_type=post_type, ranks=[clusters[0], clusters[1]]
    ).flatten()
    post_0_2 = post_obj.barcode_distance_matrix(
        data_type=post_type, ranks=[clusters[0], clusters[2]]
    ).flatten()
    post_1_2 = post_obj.barcode_distance_matrix(
        data_type=post_type, ranks=[clusters[1], clusters[2]]
    ).flatten()
    ...
    g.ax_joint.plot(
        np.linspace(max(min(dist_norm), min(post_norm)), 1, 100),
        np.linspace(max(min(dist_norm), min(post_norm)), 1, 100),
        linestyle="dashed",
